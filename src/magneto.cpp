...
//
// Converted to run on an ESP32 using vscode and platformio and the esp idf. Will probably work with other IDEs.
// Peter Martin peter@naiadhome.com
...
void magneto(int16_t (*data)[3], int16_t nlines) {
    char buf[120];
    double *D, *S, *C, *S11, *S12, *S12t, *S22, *S22_1, *S22a, *S22b, *SS, *E, *d, *U, *SSS;
    double *eigen_real, *eigen_imag, *v1, *v2, *v, *Q, *Q_1, *QB, J, hmb, *SSSS;  // *B
    int *p;
    int i, index;
    ...
    Serial.printf("\r\nMagneto 1.2\r\nInput .csv file? ");
    Serial.printf("There are %d rows\n", nlines);
   ...
   Serial.printf("%8.2lf %8.2lf %8.2lf \r\n", B[0], B[1], B[2]);
   ...
   Serial.printf("\r\nCorrection matrix Ainv, using Hm=%lf:\r\n", hm);
    for (i = 0; i < 3; i++)
       Serial.printf("%9.5lf %9.5lf %9.5lf\r\n", A_1[i * 3], A_1[i * 3 + 1], A_1[i * 3 + 2]);
